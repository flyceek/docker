...
ENV ACTIVEMQ_WOR_HOME=/opt/sfot/activemq
ENV ACTIVEMQ_HOME=${ACTIVEMQ_WOR_HOME}/${ACTIVEMQ_EXTRACT_DIR}

RUN set -x && \
    mkdir -p ${ACTIVEMQ_HOME} && \
    cd ${ACTIVEMQ_WOR_HOME} && \
    curl -S ${ACTIVEMQ_FILE_URL} | tar xvz -C ${ACTIVEMQ_WOR_HOME}
